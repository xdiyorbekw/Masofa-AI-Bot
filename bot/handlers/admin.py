...
async def _start_admin_auth(
    ...
    await message.answer(
        i18n.text(settings.default_language, "admin.ask_username")
    )
...
async def admin_cancel_command(
    ...
    if (
        data.get("admin_user_id") == message.from_user.id
        and await state.get_state() == AdminStates.authenticated.state
    ):
        add_activity_log(
            session,
    ...
    await message.answer(
        i18n.text(language, "admin.session_cancelled")
    )
...
async def admin_command_handler(
    ...
    if await _ensure_authenticated(
        user_id=message.from_user.id,
        state=state,
        settings=settings,
    ):
...
async def admin_callback_handler(
    ...
    if callback_data.action == "metrics":
        ...
        rows = await session.execute(
            ...
            .order_by(User.language_code)
        )

        language_lines = [
            i18n.text(
                language,
                "admin.metrics_language_line",
                lang=lang,
                count=count,
            )
        ...
        # username здесь не используется.
        ...
        for user in users:
            ...
            # Ссылка напрямую на профиль Telegram по user_id.
            user_link = (
            ...
            user_lines.append(f"• {user_link}")

        users_text = (
            "\n".join(user_lines)
            if user_lines
            else i18n.text(language, "admin.no_data")
        )
...
async def broadcast_callback_handler(
    ...
    if (
        ...
        != AdminStates.waiting_broadcast_confirmation.state
    ):
        await callback.answer(
            i18n.text(
                language,
                "admin.broadcast_missing",
            ),
            show_alert=True,
        )
        return
    ...
    payload = BroadcastPayload(
        ...
        entities=tuple(
            payload_data.get("entities", ())
        ),
        caption_entities=tuple(
            payload_data.get("caption_entities", ())
        ),
    )
    ...
    try:
        ...
        logger.exception(
            "Broadcast failed unexpectedly for admin user_id=%s",
            callback.from_user.id,
        )
...
async def broadcast_content_handler(
    ...
    if (
        kind in {"photo", "video"}
        and len(message.caption or "") > 1024
    ):
        await message.answer(
            i18n.text(
                language,
                "admin.broadcast_caption_too_long",
            )
        )
        return
    ...
    kind_label = i18n.text(
        language,
        f"admin.broadcast_kind.{kind}",
    )

    preview_html = (
        html.escape(preview[:500])
        if preview
        else i18n.text(
            language,
            "admin.broadcast_no_caption",
        )
    )
